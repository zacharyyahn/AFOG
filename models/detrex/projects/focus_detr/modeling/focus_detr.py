...
class FOCUS_DETR(nn.Module):
    ...
    def forward(self, batched_inputs, give_logits = False):
    ...
    def detect(self, inputs, meta=None, give_logits=False):
        formatted_inputs = [meta]
        return self.forward(formatted_inputs, give_logits)
    ...
    def compute_object_untargeted_gradient(self, x, x_orig=None, detections=None, norm=False):
        ...
        detections[0]['meta']['image'] = x_copy
        detections_adv = self.detect(x_copy, detections[0]['meta'], give_logits=True)
        target = [{"labels":detections[0]['instances'].pred_classes, "boxes":detections[0]['instances'].pred_boxes.tensor}]
        copy_mask = detections[0]['instances'].scores > 0.1
        target[0]["labels"] = target[0]["labels"][copy_mask]
        target[0]["boxes"] = target[0]["boxes"][copy_mask]
        target[0]["size"] = detections[0]["instances"].image_size
        losses = self.criterion(detections_adv, target)
        loss_ce = losses["loss_class"]
        loss_bbox = losses["loss_bbox"]
        loss_norm = torch.linalg.norm(torch.abs(x_copy - x_orig).flatten(), 5) if norm else 0.0
        #print("ATTN untarget: bbox loss:", loss_bbox.item(), "Class loss:", loss_ce.item(), "Norm loss:", loss_norm)
        unt_loss = -(loss_ce + loss_bbox / (loss_bbox / loss_ce)) + loss_norm
    ...
    def compute_object_vanishing_gradient(self, x, x_orig=None, detections=None, norm=False):
        ...
        target = [{"labels":torch.tensor([0]).long().cuda(), "boxes":torch.tensor([[0.0001, 0.0001, 0.0002, 0.0002]]).cuda()}]
        ...
        unt_loss = (loss_ce + loss_bbox/(loss_bbox/loss_ce)) + int(norm) * loss_norm
        #print("Attention loss:", unt_loss)
        #self.optimizer.zero_grad()
        unt_loss.backward()
        torch.set_grad_enabled(False)
    ...
    def compute_object_fabrication_gradient(self, x, x_orig=None, detections=None, norm=False):
        #print(" ---- ATTEMPTING TO COMPUTE ATTACK GRADIENT ---- " )
        torch.set_grad_enabled(True)
        ...
        # Use all predicted things as targets, flooding the prediction space
        losses = self.criterion(detections_adv, target)
        loss_ce = losses["loss_class"]
        loss_bbox = losses["loss_bbox"]
        loss_norm = torch.linalg.norm(torch.abs(x_copy - x_orig).flatten(), 5) if norm else 0.0
