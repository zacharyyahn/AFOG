...
class DINO(nn.Module):
    ...
    def forward(self, batched_inputs, give_logits=False):
        ...
        # modified for attack to get outputs
        if give_logits:
            return output
    ...
    def compute_object_untargeted_gradient(self, x, x_orig=None, detections=None, norm=False):
        ...
        assert torch.isfinite(x).all(), "x before copying contains infinite or NaN!"
        ...
        assert torch.isfinite(x_copy).all(), "x_copy before adv detections contains infinite or NaN!"
